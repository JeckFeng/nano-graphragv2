...
class CommunityReportsExtractor:
    ...
    async def __call__(self, input_text: str):
        ...
        try:
            ...
            supports_json = getattr(
                getattr(self._model, "config", None), "model_supports_json", False
            )
            if supports_json:
                ...
                try:
                    parsed_dict = json.loads(response.output.content)
                    parsed_response = CommunityReportResponse(**parsed_dict)
                except Exception:
                    # Fall back to no structured output if the model doesn't return valid JSON
                    parsed_response = None
