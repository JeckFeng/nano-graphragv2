...
class LitellmEmbeddingModel:
    ...
    def _log_request_size(self, inputs: list[str]) -> None:
        """Log the effective request size for diagnostics."""
        try:
            total_chars = sum(len(x) for x in inputs)
            logger.info(
                "Embedding request size: batch=%s, total_chars=%s, avg_chars=%.1f",
                len(inputs),
                total_chars,
                total_chars / len(inputs) if inputs else 0,
            )
    ...
    async def aembed_batch(
        ...
        self._log_request_size(text_list)
