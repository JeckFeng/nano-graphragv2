...
def with_rate_limiter(
    *,
    ...
    def _count_tokens(kwargs: dict[str, Any]) -> int:
        ...
        try:
            ...
            # Some providers may not yet be supported by LiteLLM token_counter; fall back to a rough estimate.
            logger.debug(
                "token_counter failed for model %s; using heuristic estimate: %s",
                model_config.model,
                exc,
            )
            payload = kwargs.get("messages") or kwargs.get("input") or ""
            token_count += max(len(str(payload)) // 4, 0)
        return token_count

    def _wrapped_with_rate_limiter(**kwargs: Any) -> Any:
        token_count = _count_tokens(kwargs)
