...
def create_math_agent() -> tuple:
    ...
    agent = create_deep_agent(
        ...
        interrupt_on={
            "multiply_numbers": True,  # 调用乘法工具时中断，等待人工审核
        },
        checkpointer=checkpointer,
        system_prompt="""你是一个数学助手。使用 multiply_numbers 工具来计算两数之积。

**严格规则**：
1. 禁止自己进行任何计算，无论多简单，必须使用 multiply_numbers 工具
2. 如果工具调用被人工审核拒绝（rejected），不要重试，直接报告操作被拒绝
3. 不要尝试用其他方式计算或编造结果"""
    )
    
    agent = wrap_runnable_with_tool_context(agent, agent_name="multiplication-agent")
