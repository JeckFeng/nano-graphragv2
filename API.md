...
- `GET /v1/artifacts/{artifact_id}?user_id=...`
  - 拉取大 JSON 或长文本（不走 WS）
...
说明：
- WS 只返回 artifact 元数据，完整 JSON 通过 REST 拉取。
...
```json
{
  ...
  "content": "...",
  "artifacts": [
    { "artifact_id": "a-001", "media_type": "application/json" }
  ]
}
```
...
- 对于大型工具结果，在服务层生成 artifact 并返回 `artifact_id`，前端通过 REST 拉取。

### 6.5 MVP 策略（你当前的取舍）
- 先实现 `final` 一次性输出，保证端到端跑通。
- 预留 `astream_events` 的事件映射入口，后续再切换到流式输出。
...
- 实现 `GET /v1/artifacts/{artifact_id}`（REST 拉取大 JSON）
...
## 10. Artifact 拉取接口格式与最小实现步骤

### 10.1 REST 接口格式（MVP）

- `GET /v1/artifacts/{artifact_id}?user_id=...`
  - 行为：返回 artifact 原始内容（JSON / 纯文本）
  - 响应头：`Content-Type` 使用 artifact 的 `media_type`
...
1. 在服务层新增 `ArtifactService`，提供 `create_artifact` 与 `get_artifact`。
2. 在 ChatService 中检测“超大 JSON”，将内容存入文件或对象存储，记录 `artifact_id` 与 `storage_path`。
3. 在 WS 的 `final` 消息里返回 `artifact_id`（不传完整 JSON）。
4. 在 `GET /v1/artifacts/{artifact_id}` 中校验 `user_id` 是否拥有该 `thread_id`。
5. 返回 artifact 的原始内容与正确的 `media_type`。
